...
- **A single money ledger** (`transactions`) for every charge, COD collection, commission, payout, service fee, delivery fee, and adjustment — amounts are always positive integers in minor units; direction is encoded by `(transaction_type, src_acc_id, dst_acc_id)`.
